...
public class AdminOrderDetails {
    ...
    @GetMapping("/{orderId}")
    public String returnAdmOrdersList(@PathVariable("orderId") String id, Model model, HttpServletRequest request) {

        //Костыль для обходя часткого запроса к БД - возвращаем вытянутый из БД объект, который в сессии.
        Order adminOrder = (Order) request.getSession().getAttribute("AdminOrder");
        if (adminOrder!=null && adminOrder.getId().equals(Integer.valueOf(id))) {
